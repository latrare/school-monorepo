...
let tests = [
  ...
  "newrefproc " >::
  (fun _ -> assert_equal
      (RefType (FuncType(IntType, IntType)))
      (chk "newref(proc (a:int) { a + 1 })"));

  "newrefpair " >::
  (fun _ -> assert_equal
      (RefType (PairType (IntType, BoolType)))
      (chk "newref(pair(0, zero?(0)))"));
  ...
  (fun _ -> assert_equal
      ...
      (chk "deref(newref(proc (a:int) { a + 1}))"));

  "derefpair " >::
  (fun _ -> assert_equal
      (PairType(IntType, BoolType))
      (chk "deref(newref(pair(0, zero?(0))))"));

  "setref " >::
  (fun _ -> assert_equal
      (UnitType)
      (chk "setref(newref(0), 3)"));

  "setrefproc " >::
  (fun _ -> assert_equal
      (UnitType)
      (chk "setref(newref(proc (x:int) { x + 1 }), proc (x:int) { x + 2})"));

  "setrefpair " >::
  (fun _ -> assert_equal
      (UnitType)
      (chk "setref(newref(pair(0, zero?(0))), pair(1, zero?(1)))"));
